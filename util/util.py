...
def save_image(image_numpy, image_path, create_dir=False, aspect_ratio=1.0):
    if create_dir:
...
def load_network(net, label, epoch, opt, use_strict=True):
    save_filename = '%s_net_%s.pth' % (epoch, label)
    save_dir = os.path.join(opt.checkpoints_dir, opt.name)
    save_path = os.path.join(save_dir, save_filename)
    weights = torch.load(save_path)
    net.load_state_dict(weights, strict=use_strict)
    print(f'loaded:{save_filename}')
    return net


def load_pretrained_network(net, label, epoch, opt, use_strict=True):
    save_filename = '%s_net_%s.pth' % (epoch, label)
    save_dir = os.path.join(opt.checkpoints_dir, opt.pretrained_name)
    save_path = os.path.join(save_dir, save_filename)
    weights = torch.load(save_path)
    net.load_state_dict(weights, strict=use_strict)
    return net
